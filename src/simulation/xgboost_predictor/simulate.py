...
def simulate_data_with_xgboost(df, model_name, fit_year, ground_truth_column, area_code, year, full_output_path):
    project_root = Path(__file__).resolve().parents[3]
    base_path = project_root / get_model_output_base() / "xgboost_output"

    age_groups = df["age_group"].unique()
    all_predictions = []
    for age_group in age_groups:
        df_prediction = df[df["age_group"] == age_group]
        path = Path(f"{base_path}/{model_name}/{fit_year}/pipelines/best_pipeline_{age_group}.joblib").resolve()
        pipe = load(path)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run XGBOOST simulation in isolated environment")
    parser.add_argument("--df", required=True, help="Input parquet file with features")
    parser.add_argument("--model-name", required=True, help="Path to XGBOOST model directory")
    parser.add_argument("--fit-year", required=True, help="Path to XGBOOST model directory")
    ...
    result = simulate_data_with_xgboost(
        df=df,
