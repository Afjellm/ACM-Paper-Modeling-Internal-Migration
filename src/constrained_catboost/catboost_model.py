...
from src.constrained_catboost.experiment import run_experiment_and_write_results
...
from src.constrained_catboost.utils import load_monotone_constraints_from_xlsx

# Training hyperparameters
GROUND_TRUTH_COLUMN = "amount"
NORMALIZE_COUNT_COLS=True
constraint_set = "s1"
EXPERIMENT_NAME = f"caboost_{constraint_set}"
START_YEAR = 2018
...
if __name__ == "__main__":
    ...
    print(f"Training catboost boost model using ground truth column {GROUND_TRUTH_COLUMN} in output path {EXPERIMENT_NAME}")
    ...
    categorical = ["rural_level_origin", "rural_level_target", "is_covid_year", "is_adjacent"]

    features_to_train_on = features_for_training_numeric + categorical

    preprocessor_cb = ColumnTransformer(
        transformers=[
            ("num", "passthrough", features_for_training_numeric),
    ...
    for year, df in data.items():
        ...
        for age_group in age_groups:
            ...
            X_Data[year][age_group] = df_age[[col for col in features_to_train_on if col in df.columns]]
    ...
    for fit_years in fit_years:
        for age_group in age_groups:
            ...
            monotone_constraints = load_monotone_constraints_from_xlsx(
                ...
                features_for_training_numeric=features_for_training_numeric,
                age_col="age group")

            preprocessor_cb.fit(X_Data[2019][age_group])
            feature_names_out = preprocessor_cb.get_feature_names_out()

            monotone_list = [
                monotone_constraints.get(base_name(fname), 0)
                for fname in feature_names_out
            ]

            catboost_reg = Pipeline(steps=[
            ...
            run_experiment_and_write_results(fit_years, X_Data, Y_Data, all_data_restructured, grid_search,
                                             ...
                                             END_YEAR_EXCLUSIVE, age_group,Y_AMOUNT_DATA)
