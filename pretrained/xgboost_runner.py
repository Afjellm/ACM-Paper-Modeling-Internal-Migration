...
def predict_full_results(df: pd.DataFrame, prediction_year: int) -> pd.DataFrame:
    details = get_model_artifact_details("xgboost")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Recreate XGBoost predictions from pretrained artifacts.")
    parser.add_argument(
        "--overwrite-inputs",
        action="store_true",
...
def main() -> None:
    ...
    print_written_paths("xgboost", written)
